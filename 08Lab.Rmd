...
eucDist<- vegdist(penguins_noNA[penguins_noNA$species=='Adelie',3:6],"euclidean")
...
#Chinstrap
eucDist<- vegdist(penguins_noNA[penguins_noNA$species=="Chinstrap",3:6],"euclidean")
multOut<-scale(colMeans(as.matrix(eucDist)))
hist(multOut)
...
eucDist<- vegdist(penguins_noNA[penguins_noNA$species=="Gentoo",3:6],"euclidean")
multOut<-scale(colMeans(as.matrix(eucDist)))
hist(multOut)
...
Check the frequency of each species in the training data, which will help set your priors proportional to the number of samples in each species group:
...
```{r}
penguinsLDA <- lda(species ~ bill_length_mm + bill_depth_mm + flipper_length_mm, penguins_noNA, prior = prior, subset = train)
```
...
```{r}
penguinsmod <- lm(cbind(bill_length_mm, bill_depth_mm, flipper_length_mm) ~ species, data=penguins_noNA[train, ])
```
...
```{r}
YAdelie<-as.matrix(penguins_noNA[penguins_noNA$species=='Adelie',3:5])
YChinstrap<-as.matrix(penguins_noNA[penguins_noNA$species=='Chinstrap',3:5])
YGentoo<-as.matrix(penguins_noNA[penguins_noNA$species=='Gentoo',3:5])
Sp<-factor(penguins_noNA$species)
